...
import { getDatabase, ref, set, remove as dbRemove } from "firebase/database";
import { 
  getAuth, 
...
const SignUpAssociateDentist = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // (No pendingKey logic since we want email always empty)
    } catch (error) {
    ...
    if (showVerifyModal) {
      interval = setInterval(async () => {
        ...
        if (auth.currentUser.emailVerified) {
          ...
          await set(ref(db, `users/Personnel/AssociateDentist/${auth.currentUser.uid}/emailVerified`), "Yes");
